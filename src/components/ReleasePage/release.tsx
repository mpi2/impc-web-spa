...
const listOfPastReleasesPage = ["22.1", "22.0"];
...
const ReleaseNotesPage = ({ releaseVersion }: Props) => {
  const releaseMetadata = !!releaseVersion
    ? releaseMetadataMap[releaseVersion]
    : releaseMetadataMap["latest"];
  ...
  const formatDate = (date: string) => {
    const dateObj = parse(date, "d-MM-yyyy", new Date());
